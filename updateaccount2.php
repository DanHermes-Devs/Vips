...
$resreg=$mysqli->query("SELECT * FROM registros WHERE id_registro = $uid OR facebook_id = $uid OR google_id = $uid");
	...
	if ( isset($_POST['correo']) ) {
		...
		if($correo == $mail){
			...
			$querync = "SELECT * FROM registros WHERE correo= '".$mail."'";

		$resulttc = $mysqli->query($querync) or die("Falló la consulta $queryrr");


		$numfilasc = $resulttc->num_rows;

		 if( 	$numfilasc == 1){
			 
			 echo "<div class='errormsg'>El correo electrónico ya está asociado con otro usuario.</div>";
			die();
		...
		if($tel == $telefono){
				 ...
				 $queryn = "UPDATE registros SET nombre   = '$nombre',
						   ...
						   WHERE id_registro = $uid OR facebook_id= $uid";
				...
				if($result){echo "<div class='success'>Datos guardados correctamente.". $switch."</div>";}
