...
$consulta = $mysqli->query("SELECT * FROM  `registros`
	WHERE	 password = 'lead' AND tdl IS NULL AND LENGTH(nombre) > 3 AND LENGTH(apellido) > 3 AND LENGTH(correo) > 5
    LIMIT    1");
...
$wsdl = 'https://soaprod.grupoalsea.com.mx/Services/Loyalty/ALSEAMemberService?wsdl';
			...
			if(strlen($consul) > 6){
				...
				echo '<title>'.$uid.' TDL Actualizado.</title> <meta http-equiv="refresh" content="2; URL=https://vips.com.mx/recuperaWowLead">';
