...
if($edo == 'ubicacion'){
	...
	$sql = "SELECT id, name, addressresume, address, zip, city, state, country, url, delivery,  new, ninos, phone, latitude, longitude, horario, alcohol, remodelado, estacionamiento, salon, terraza, uber, sdelantal, rappi, storeNumber,
		...
		FROM stores WHERE WHERE desactivar = 0
		HAVING distance <= 3.5
...
if( $res ) {
	...
	while( $store = @mysqli_fetch_assoc($res) ) {
		...
		$store['botonmaps'] = '<a onclick="ClicLocationGoogle(&quot;'.$store['id'].'&quot;, &quot;'.$store['storeNumber'].'&quot;, &quot;'.$store['name'].'&quot;, &quot;Listado de restaurantes&quot;);" style="position: relative; left: 0; width: 100%; margin-top: 0; padding: 0; display: inline-block; margin-right: 70%;" target="_blank" href="'.$store['linkmaps'].'"><p class="link1">Llévame ahí</p> </a>	';
		...
		$store['linkmaps'] = "https://waze.com/ul?ll=".$store['latitude'].",".$store['longitude']."&navigate=yes";
