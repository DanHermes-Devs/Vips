...
(function($) {
	...
	function placeMarkerForStore(store, bounds) {
		var id = store.id;
		var image = 'https://vips.com.mx/ubicaciones/icon_ubi.png';
	...
	function openInfoWindowForStore(store) {
		...
		analytics.track("Location pin selected", {
		  id: store.id, 
		  location_data: JSON.stringify(store),
		});
	}
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
      var pos = {
	   ...
	   var marker = new google.maps.Marker({
			position: pos,
			icon: new google.maps.MarkerImage('https://vips.com.mx/ubicaciones/icon-aqui.png'),
    ...
    if(e.keyCode == 13) {
		...
		setTimeout(function(){
			analytics.track("Location Searched", {
				  texto: $("#searchbox").val(),
				  location: $(".mm-listview .title:eq( 1 )").text()		  
			});
			//alert($(".mm-listview .title:eq( 1 )").text());
		},8000)
    }
});
