...
if( isset($_SESSION['uservips'])!="" ){
	...
	$numfilas1n = $result1n->num_rows;if($numfilas1n ==1){
		while($row1n= mysqli_fetch_array($result1n)){
			$cp = $row1n["cp"];
			}
		header('Location: domicilio2-m?cp='.$cp);
		exit(0);
	}else{
...
$detect = new Mobile_Detect();if ($detect->isMobile()) {
	...
	header("Location: /domicilio");exit(0);

}?><div class="main"><?php include('lateral.php');?><main role="main">
...
<script type="text/javascript">
		function tilter() {	//	default opts:
		//	parallaxMultiple: 0.1,
		//	tiltMultiple: 0.05,
		//      duration: 500,
		//      easing:'easeOutExpo'        $('.tilter_image').parallaxTilt({
              parallaxMultiple: 0.1,
				tiltMultiple: 0.05,
				duration: 800,
				easing:'easeOutExpo'
